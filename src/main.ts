...
import { dumpGitHubEventPayload, getChangelogOptions } from './utils';
import {
  isBreakingChange,
...
type Args = {
  apiToken: string;
  releaseTag: string;
  compareLastTaggedRelease: boolean;
  preRelease: boolean;
  releaseTitle: string;
  files: string[];
};
...
const getAndValidateArgs = (): Args => {
  const args = {
    ...
    releaseTag: core.getInput('release_tag', { required: false }),
    compareLastTaggedRelease: core.getInput('compare_last_tagged_release', { required: false }) === 'true',
...
const createReleaseTag = async (client: GitHub, refInfo: Octokit.GitCreateRefParams) => {
...
const deletePreviousGitHubRelease = async (client: GitHub, releaseInfo: Octokit.ReposGetReleaseByTagParams) => {
...
const searchForPreviousReleaseTag = async (
  ...
  const tagList = tl
    .map((tag) => {
      // map all tags to semver versions
    ...
    .filter((tag) => tag.semverTag !== null) // filter out any tags that don't conform to semver
    .sort((a, b) => semverRcompare(a.semverTag, b.semverTag)); // sort tags in descending order
  ...
  if (validSemver !== null) {
    ...
    if (tagList.length > 0) {
      previousReleaseTag = tagList[0].name; // take the first (latest) valid semver tag if the current tag is not a valid semver
    }
  }
...
export const main = async (): Promise<void> => {
  try {
    ...
    const client = new GitHub(args.apiToken, {
    ...
    core.startGroup('Collecting Github Context');
    dumpGitHubEventPayload();
    core.debug(`Github context: ${JSON.stringify(context)}`);
    core.endGroup();

    core.startGroup('Determining release tags');
    const releaseTag = args.releaseTag ? args.releaseTag : parseGitTag(context.ref);
    if (!releaseTag) {
      throw new Error(
        `The parameter "release_tag" was not set and this does not appear to be a GitHub tag event. (Event: ${context.ref})`,
      );
    }
    core.info(`Release tag: ${releaseTag}`);

    let previousReleaseTag = args.releaseTag;
    if (!previousReleaseTag || args.compareLastTaggedRelease) {
      previousReleaseTag = await searchForPreviousReleaseTag(client, releaseTag, {
    ...
    core.debug(`Exporting environment variable RELEASE_TAG with value ${releaseTag}`);
